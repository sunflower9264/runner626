...
*   **核心体验**: 这是一个 **1v1 的双人竞速游戏**。玩家不仅要面对地形和敌人的挑战，还要与一名 AI 控制的对手（Rival）进行实时赛跑。核心目标是比对手先到达终点。
*   **视觉风格**: 像素艺术风格，带有科幻/机甲元素。背景采用多层视差滚动。
...
*   **跳跃 (Jump)**:
    *   核心操作。支持定高跳跃，部分形态可能支持二段跳或飞行（如 Rocket 形态）。
    *   物理模拟采用定点数逻辑，重力加速度固定。
*   **下蹲/滑铲 (Crouch/Slide)**:
    *   用于躲避高处的障碍物或飞行的敌人。
    *   改变角色的碰撞箱高度。
...
### 2.2 战斗系统
*   **攻击 (Attack)**:
    *   角色可以发射投射物（子弹）或进行近战攻击。
    *   攻击动作会打断当前的奔跑动画，但通常允许在移动中进行。
    *   **干扰对手**: 攻击不仅可以消灭普通敌人，可能也可以干扰 AI 对手（待确认，通常此类游戏允许攻击对手使其减速）。
*   **碰撞伤害**:
    *   接触敌人本体会受到伤害并产生击退效果（Knockback）。
